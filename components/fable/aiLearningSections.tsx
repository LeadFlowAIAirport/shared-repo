...
import { aiLearningPage } from "@/lib/fablePages";
import { aiImplementation, type LessonVideo } from "@/lib/content";

/* The Fable AI Learning sections, shared by the live /ai-implementation page
   and the /preview/fable-ai-learning route. Copy lives in lib/fablePages.ts
   (aiLearningPage); the lesson video library, implementation modules, and FAQ
   render from lib/content.ts (`aiImplementation`), so recorded videos keep
   swapping in from one place (set a lesson's `video.status`/`videoUrl`) and
   the module cards keep their /services#<id> deep links. */
...
/** "Coming soon" / "Watch" chip — same state logic as the old learning hub:
...
export function AilHero() {
  return <PageHero {...hero} />;
}

/* 01 — Why AI feels confusing */
export function AilProblemSection() {
  return (
...
export function AilTeachSection() {
  return (
    <Shell id="teach">
      <Kicker index="02" label={teach.kicker} />
      <div className="mt-8 flex flex-wrap items-end justify-between gap-6">
        <Reveal>
          <h2 className={`fbl-display max-w-2xl text-ink ${HEADING_SIZE}`}>{teach.heading}</h2>
        </Reveal>
        <Reveal delay={80}>
...
export function AilPathSection() {
  return (
    <Shell id="path" deep>
...
/* 04 — Mini lesson library (video slots shared with lib/content.ts) */
export function AilLibrarySection() {
  const { featured, lessonsIntro, lessonsNote, lessons } = aiImplementation;
  return (
    <Shell id="lessons">
...
export function AilModulesSection() {
  const { modules: moduleItems } = aiImplementation;
  return (
    <Shell id="modules" deep>
      ...
      <Reveal delay={160}>
        <Link href={modules.servicesLink.href} className="fbl-btn fbl-btn-ghost fbl-btn-sm mt-8">
...
/* 06 — What this is not */
export function AilHonestySection() {
  return (
    <Shell id="honesty">
            ...
            <li key={n.title} className={i > 0 ? "border-t border-white/7" : ""}>
              <Reveal delay={i * 60}>
                <div className="flex items-start gap-5 py-5">
                  <span
                    className="fbl-mono pt-0.5 text-sm font-medium text-accent/60"
                    aria-hidden
                  >
                    ✕
                  </span>
...
/* 07 — FAQ (data shared with lib/content.ts) */
export function AilFaqSection() {
  const { faq } = aiImplementation;
  return (
    <Shell id="faq" deep>
